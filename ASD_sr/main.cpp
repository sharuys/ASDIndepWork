...
/*Дано натуральне число n. Від даного числавіднімітьсуму цифр цього числа,
 ...
 * Порахувати скільки разів будевиконанаданаоперація.*/
...
/*
 * Подвійнийфакторіал–це функція,  яка  дорівнює  добутку  непарних натуральних  чисел  від  1  до N,
...
void findAndPrintPerfectNumbers(int N) {
    cout << "Досконалі числа від 1 до " << N << ": ";
    ...
    for (int i = 2; i <= N; i++) {
        ...
        for (int j = 2; j * j <= i; j++) {
            if (i % j == 0) {
                sum += j; // Додаємо дільник
                if (j != i / j) {
                    sum += i / j; // Додаємо парний дільник
                }
            }
        }

        // Перевірка, чи число є досконалим та вивід результату
        if (sum == i) {
            cout << i << " ";
        }
    }
